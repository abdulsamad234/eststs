...
class _SelectReceiverPageState extends State<SelectReceiverPage> {
  ...
  getPlayers() {
    print('Receiver players in page: ${widget.players}');
  ...
  onChangeText(String newText) {
    if (widget.title == 'Select a Receiver') {
      if (newText == '') {
        currentSelectedPlayer == null
            ? setState(() {
                actionData.removeWhere((key, value) {
                  return key == 'receiverNumber';
                });
              })
            : setState(() {
                actionData['receiverNumber'] = currentSelectedPlayer.number;
              });
      } else {
        setState(() {
          actionData['receiverNumber'] = int.parse(newText);
        });
      }
    }
    else{
      if (newText == '') {
        currentSelectedPlayer == null
            ? setState(() {
                actionData.removeWhere((key, value) {
                  return key == 'returnerNumber';
                });
              })
            : setState(() {
                actionData['returnerNumber'] = currentSelectedPlayer.number;
              });
      } else {
        setState(() {
          actionData['returnerNumber'] = int.parse(newText);
        });
      }
    }
